...
class TestBasicFunctionality:
    ...
    def test_tool_parser_selection(self):
        """Test tool parser selection logic."""
        devstral_parser = get_tool_parser("devstral")
        assert devstral_parser is not None

        qwen_parser = get_tool_parser("qwen3")
        assert qwen_parser is not None

        # Default should be qwen3 based on the code
        default_parser = get_tool_parser("unknown-model")
